...
from accounts.models import LoanAccount, LoanApplication, MemberAccount, User
...
class MemberLoansTests(MemberPortalTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        app1 = LoanApplication.objects.create(
            application_number="LA-MEM-TEST-001",
            user=self.member_user,
            loan_type="personal",
            ...
            status="approved",
            approval_date=date(2025, 1, 2),
            created_by=self.admin_user,
        )
        self.member_loan = LoanAccount.objects.create(
            application=app1,
            user=self.member_user,
            loan_number="LN001",
            status="active",
            ...
            tenure_months=24,
            emi_amount=Decimal("4634.00"),
            total_payable=Decimal("111216.00"),
            total_paid=Decimal("0"),
            outstanding_balance=Decimal("111216.00"),
            total_emis=24,
            emis_paid=0,
            emis_overdue=0,
            created_by=self.admin_user,
        ...
        app2 = LoanApplication.objects.create(
            application_number="LA-MEM-TEST-002",
            user=self.other_member,
            loan_type="home",
        ...
        self.other_loan = LoanAccount.objects.create(
            application=app2,
            user=self.other_member,
            loan_number="LN002",
            status="active",
            ...
            total_payable=Decimal("600000.00"),
            total_paid=Decimal("0"),
            outstanding_balance=Decimal("600000.00"),
            total_emis=60,
            emis_paid=0,
            emis_overdue=0,
            created_by=self.admin_user,
    ...
    def test_loan_detail_blocks_other_members_loan(self):
        ...
        self.client.login(username="member_test", password="member12345")
        response = self.client.get(reverse("member_portal:loan_detail", args=["account", self.other_loan.id]))
        self.assertEqual(response.status_code, 404)
