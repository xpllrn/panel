...
from accounts.models import LoanAccount, LoanApplication, LoanRepayment, MemberAccount, Transaction
...
def member_dashboard_view(request):
    ...
    recent_transactions = Transaction.objects.filter(user=user).select_related("member_account")[:5]

    active_loans = LoanAccount.objects.filter(user=user, status="active").select_related("application")
    total_outstanding = sum(loan.outstanding_balance for loan in active_loans)

    share_capital = user_active_share_capital_total(user)
    number_of_shares = user_active_share_count(user)
    dividend_payable = user.dividend_payable_balance

    # Dividend transactions
    dividend_transactions = Transaction.objects.filter(user=user, transaction_type="dividend").order_by("-created_at")[:5]
...
def member_account_detail_view(request, account_id):
    ...
    transactions_qs = Transaction.objects.filter(member_account=account).order_by("-created_at")
...
def _application_total_payable(app):
...
class MemberLoanBridge:
    """Uniform loan row for member loan_detail template (account or open application)."""

    __slots__ = ("_ac", "_app")

    def __init__(self, *, account=None, application=None):
        if bool(account) == bool(application):
            raise ValueError("MemberLoanBridge: pass exactly one of account= or application=")
        self._ac = account
        self._app = account.application if account else application

    @property
    def loan_number(self):
        return self._ac.loan_number if self._ac else self._app.application_number

    def get_loan_type_display(self):
        return self._app.get_loan_type_display()

    def get_status_display(self):
        return self._ac.get_status_display() if self._ac else self._app.get_status_display()

    @property
    def status(self):
        return self._ac.status if self._ac else self._app.status

    @property
    def principal_amount(self):
        return self._app.principal_amount

    @property
    def interest_rate(self):
        return self._app.interest_rate

    def get_interest_type_display(self):
        return self._app.get_interest_type_display()

    @property
    def tenure_months(self):
        return self._app.tenure_months

    @property
    def emi_amount(self):
        if self._ac:
            return self._ac.emi_amount
        return self._app.calculate_emi()

    @property
    def total_payable(self):
        if self._ac:
            return self._ac.total_payable
        return _application_total_payable(self._app)

    @property
    def total_paid(self):
        return self._ac.total_paid if self._ac else Decimal("0")

    @property
    def outstanding_balance(self):
        if self._ac:
            return self._ac.outstanding_balance
        return self._app.principal_amount

    @property
    def overdue_amount(self):
        return self._ac.overdue_amount if self._ac else Decimal("0")

    @property
    def emis_paid(self):
        return self._ac.emis_paid if self._ac else 0

    @property
    def total_emis(self):
        return self._ac.total_emis if self._ac else self._app.tenure_months

    @property
    def completion_percentage(self):
        if self._ac:
            return self._ac.completion_percentage
        return 0

    @property
    def application_date(self):
        return self._app.application_date

    @property
    def approval_date(self):
        return self._app.approval_date

    @property
    def disbursement_date(self):
        return self._ac.disbursement_date if self._ac else None

    @property
    def purpose(self):
        return self._app.purpose or ""


@member_required
def member_loans_view(request):
    """List loan accounts and open applications for the member."""
    user = request.user
...
@member_required
def member_loan_detail_view(request, loan_kind, loan_id):
    """View a loan account or application with repayment schedule when applicable."""
    user = request.user
    if loan_kind == "account":
        ac = get_object_or_404(LoanAccount.objects.select_related("application"), id=loan_id, user=user)
        repayments = LoanRepayment.objects.filter(loan_account=ac).order_by("installment_number")
        loan = MemberLoanBridge(account=ac)
    elif loan_kind == "application":
        app = get_object_or_404(LoanApplication, id=loan_id, user=user)
        existing = LoanAccount.objects.filter(application=app).first()
        if existing:
            return redirect("member_portal:loan_detail", loan_kind="account", loan_id=existing.id)
        repayments = []
        loan = MemberLoanBridge(application=app)
    else:
        raise Http404("Invalid loan type")
...
def member_transactions_view(request):
    ...
    user = request.user
    transactions_qs = Transaction.objects.filter(user=user).select_related("member_account").order_by("-created_at")
...
def member_profile_view(request):
    ...
    context = {
        "member": user,
        "member_kyc": user_kyc(user),
        "member_address_current": user_address(user, "current"),
        "member_nominee_primary": nominee_primary(user),
...
def member_account_statement_view(request, account_id):
    ...
    txn_data = [["Date", "Transaction #", "Type", "Description", "Amount", "Balance"]]
